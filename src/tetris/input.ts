...
export abstract class IInputSource {
  /// Return the current keystate
  abstract keystate(game: Game): number

  read(game: Game): Input {
    const currentKeystate = this.keystate(game)
    const newKeystate = currentKeystate & ~game.input.keystate
    const newKeyCount = popcount(newKeystate)
    ...
    //
    // TODO: Takes a frame to DAS charge on the current. Trigger on piece entry
    // if we are over the limit as well.
    //
    // TODO: We currently do not prioritize any direction. This gives us the
    // nice feature that a single-tap from a DAS to wall can occur even if the
    // DAS direction is still held. This feels a bit different to other games
    // and still need to determine if it works best.
    ...
    // TODO: Could we merge rotation and movement finesse on the same frame
    // since technically it isn't a wasted actions?
...
export class KeyboardSource extends IInputSource {
  keystate(game: Game): number {
...
export class ReplaySource extends IInputSource {
  // First item is ticks, second item is keystate
  inputs: number[][]
  index: number
  ...
  keystate(game: Game): number {
    if (game.ticksAll === this.inputs[this.index + 1][0]) {
      this.index += 1
    }
